  ...
  const availability = Object.fromEntries(
    ...
    const schedulePtWithPrimaries = (patient: Patient, patientsLeftToSchedule: Patient[]) => {
      if (primaries.length === 2) {
        ...
        const possibleSlots1 = overlappingAvailability(patient, primary1);
        const possibleSlots2 = overlappingAvailability(patient, primary2);
        const sameSlot = possibleSlots1.length === 1 && possibleSlots2.length === 1 && possibleSlots1[0] === possibleSlots2[0];
        if (possibleSlots1.length < 1 || possibleSlots2.length < 1 || sameSlot) return false; // base case

        addAppointment(primary1.name, patient.name, possibleSlots1[0]);
        ...
        if (possibleSlots.length < 1) return false; // base case
      ...
      if (finished) return true; // success
      else schedulePtWithPrimaries(patientsLeftToSchedule.shift(), patientsLeftToSchedule);
    };
